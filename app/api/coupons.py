...
def list_coupons(
    active_only: bool = Query(default=True, description="Only return coupons that are active and within their valid date window."),
    ...
    if active_only:
        result = [c for c in result if _is_currently_valid(c) and (c.remaining_claims is None or c.remaining_claims > 0)]
    return result
...
def create_coupon(payload: schemas.CouponCreate, db: Session = Depends(get_db), _: None = Depends(require_admin)):
...
def update_coupon(
    coupon_id: int, payload: schemas.CouponUpdate, db: Session = Depends(get_db), _: None = Depends(require_admin)
):
...
def claim_coupon(coupon_id: int, db: Session = Depends(get_db)):
    ...
    if not coupon.is_active:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="This coupon is no longer active")
    if not _is_currently_valid(coupon):
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="This coupon is not valid yet or has expired")
    remaining = _remaining_claims(coupon)
    if remaining is not None and remaining <= 0:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="This coupon has reached its claim limit")
...
def delete_coupon(coupon_id: int, db: Session = Depends(get_db), _: None = Depends(require_admin)) -> Response:
    coupon = get_or_404(db, models.Coupon, coupon_id, "Coupon")
    db.delete(coupon)
    db.commit()
